...
export async function renderTax(view) {
  ...
  <div class="page-head">
    <div class="row"><h1>Tax</h1>
      <select id="tax-year" aria-label="Tax year">${years.map((y) => opt(y, `${y}`, y === year)).join('')}</select></div>
  ...
  <div class="tiles">
    <div class="tile"><div class="k">Company income ${year}</div><div class="v">${PLN(revenue, W)}</div><div class="muted small">${t.vat_registered ? 'net of VAT' : 'as received'}</div></div>
    <div class="tile"><div class="k">Deductible costs</div><div class="v">${PLN(costs, W)}</div><div class="muted small">${nonDed ? `+ ${PLN(nonDed, W)} not deductible` : 'lower the profit that gets taxed'}</div></div>
    <div class="tile"><div class="k">Taxable profit</div><div class="v ${profit < 0 ? 'neg' : ''}">${PLN(profit, W)}</div><div class="muted small">CIT at ${pct(citRate)} ≈ ${PLN(citSoFar, W)}</div></div>
    <div class="tile"><div class="k">Still to set aside</div><div class="v ${owed - paid > 0 ? 'neg' : ''}">${PLN(Math.max(0, owed - paid), W)}</div><div class="muted small">owed so far minus tax paid</div></div>
  </div>

  <div class="grid2">
    <section class="card">
      <h3>Tax pot, ${year} so far</h3>
      <div class="row between li"><span>Corporation tax (CIT ${pct(citRate)} of ${PLN(Math.max(0, profit), W)})</span><strong>${PLN(citSoFar)}</strong></div>
      ${t.vat_registered ? `<div class="row between li"><span>VAT to pay <span class="muted small">(${PLN(sum('vatOut'), W)} charged − ${PLN(sum('vatIn'), W)} on costs)</span></span><strong class="${vatNet < 0 ? 'pos' : ''}">${vatNet < 0 ? `${PLN(-vatNet)} refund` : PLN(vatNet)}</strong></div>` : ''}
      ${t.sole_shareholder ? `<div class="row between li"><span>ZUS <span class="muted small">(${activeMonths} month${activeMonths === 1 ? '' : 's'} × ${PLN(zus.total)}; paid by you personally)</span></span><strong>${PLN(zusSoFar)}</strong></div>` : ''}
      <div class="row between li"><span><strong>Owed so far</strong></span><strong>${PLN(owed)}</strong></div>
      <div class="row between li"><span>Tax & ZUS payments recorded <span class="muted small">(category “Tax & ZUS payments”)</span></span><strong>− ${PLN(paid)}</strong></div>
      <div class="row between li"><span><strong>Keep aside now</strong></span><strong class="${owed - paid > 0 ? 'neg' : 'pos'}">${PLN(Math.max(0, owed - paid))}</strong></div>
      <p class="muted small">If the ${PLN(afterCit, W)} left after CIT${nonDed ? ` (${PLN(Math.max(0, cashLeft), W)} after non-deductible spending)` : ''} were paid out as a dividend, ${pct(R.dividendTax, 0)} dividend tax would take another ${PLN(Math.max(0, cashLeft) * R.dividendTax, W)}.${boardYTD ? ` Board pay so far: ${PLN(boardYTD, W)} (about ${PLN(boardTax, W)} of that goes on your health contribution and PIT).` : ''}</p>
    </section>

    <section class="card">
      <h3>Coming up</h3>
      ${soon.map((d) => { const a = amountFor(d); return `<div class="row between li"><div><strong>${esc(d.title)}</strong> <span class="muted small">${niceDate(d.date)}${d.date !== d.nominal ? ' (moved from a weekend or holiday)' : ''}</span><div class="muted small">${esc(d.detail)}</div></div>${a != null ? `<span class="nowrap">${a > 0 ? PLN(a) : a < 0 ? `${PLN(-a)} back` : '<span class="muted">nothing</span>'}</span>` : ''}</div>`; }).join('') || '<p class="muted">Nothing due. Set the company registration date in Tax settings.</p>'}
    </section>
  </div>
  ...
  <section class="card">
    ...
    ${adv.filter((m) => m.month >= Math.min(startMonth, 12) && (m.month <= monthNow || m.revenue || m.costs)).map((m) => `<tr><td>${monthName(m.month)}</td><td class="r">${PLN(m.revenue, W)}</td><td class="r">${PLN(m.costs, W)}</td><td class="r ${m.profit < 0 ? 'neg' : ''}">${PLN(m.profit, W)}</td><td class="r">${m.isDue ? PLN(m.advance, W) : '<span class="muted">–</span>'}</td>${t.vat_registered ? `<td class="r">${PLN(months[m.month - 1].vatOut - months[m.month - 1].vatIn, W)}</td>` : ''}${t.sole_shareholder ? `<td class="r">${PLN(zus.total)}</td>` : ''}</tr>`).join('')}
    </tbody></table>
  ...
  <section class="card">
    <details><summary><strong>All ${year} deadlines</strong> <span class="muted small">(${yearList.length})</span></summary>
      ...
      ${yearList.map((d) => `<tr class="${d.date < td ? 'muted' : ''}"><td class="nowrap">${niceDate(d.date)}</td><td><strong>${esc(d.title)}</strong><div class="small muted">${esc(d.detail)}</div></td><td class="r nowrap">${(() => { const a = amountFor(d); return a > 0 ? PLN(a, W) : ''; })()}</td></tr>`).join('')}
      </tbody></table>
  ...
  <section class="card">
    <details><summary><strong>How the rules work (${year})</strong></summary>
      <ul class="small">
        ...
        <li><strong>ZUS</strong>: as the only shareholder you pay ${PLN(zusMonthly(year).social)} social (${PLN(zusMonthly(year, { sickness: false }).social)} without voluntary sickness cover) + ${PLN(R.zus.health)} health each month, whatever the company earns. There's no start-up discount for a sp. z o.o. It starts from the day the company is entered in KRS, and the first month is pro-rated. It's your personal cost, not the company's. With two or more shareholders, this doesn't apply.</li>
        ...
        <li><strong>Board pay (powołanie)</strong> is a company cost, so it cuts CIT. You pay ${pct(R.pit.healthOnBoardPay, 0)} health on it and PIT at ${pct(R.pit.low, 0)} / ${pct(R.pit.high, 0)} with the ${PLN(R.pit.reduction / R.pit.low, W)} tax-free amount, after ${PLN(R.pit.boardCostsMonthly, W)}/month costs. Your shareholder social ZUS can be deducted from this income. There's no social ZUS on board pay.</li>
...
function renderComparison(el, t, zusMonths) {
  ...
  el.innerHTML = `
    ...
    <table class="tbl compact"><thead><tr><th></th>${cols.map(([n]) => `<th class="r">${n}</th>`).join('')}</tr></thead><tbody>
      ${row('Board pay (gross)', (x) => PLN(x.board, W))}
      ${row('Health on board pay (9%)', (x) => PLN(x.pay.health, W))}
      ${row('PIT on board pay', (x) => PLN(x.pay.pit + x.pay.solidarity, W))}
      ${row(`CIT (${pct(opts.citRate)})`, (x) => PLN(x.cit, W))}
      ${row('Dividend paid (gross)', (x) => PLN(x.dividend, W))}
      ${row('Dividend tax (19%)', (x) => PLN(x.divTax, W))}
      ${t.sole_shareholder ? row(`Your ZUS (${zusMonths} months)`, (x) => PLN(x.zusTotal, W)) : ''}
      ${row('<strong>You keep</strong>', (x) => `<strong>${PLN(x.net, W)}</strong>`)}
      ${row('per month', (x) => PLN(x.net / 12, W), 'muted')}
      ${row('Total tax & ZUS rate', (x) => pct(x.effective))}
    </tbody></table>
    ${P > 0 ? `<p class="small">${gain > 1 ? `Paying yourself <strong>${PLN(best.board, W)}</strong> board pay (${PLN(best.board / 12, W)}/month) and the rest as dividend leaves you <strong>${PLN(gain, W)} more</strong> a year than dividends alone.` : 'Taking everything as a dividend comes out best at this profit.'}
    ...
    <div class="row between"><p class="muted small">Board pay needs a shareholder resolution (uchwała) and must be a reasonable amount for the work done. The tax office can challenge pay that's clearly out of line.</p>
      <button class="btn small" id="cmp-save">Plan ${PLN(mine / 12, W)}/month board pay</button></div>`;
  ...
  lineChart($('#c-cmp', el), labels, [{ name: 'You keep, by board pay', values: vals }], (v, short) => PLN(v, { whole: short }));
...
function editTax(done) {
  ...
  <form id="tf" class="form">
    ...
    <label class="check"><input type="checkbox" name="sickness" ${t.sickness ? 'checked' : ''}> Include voluntary sickness insurance (${PLN(z.social)} instead of ${PLN(zn.social)} social + ${PLN(z.health)} health)</label>
    <div class="row2">
